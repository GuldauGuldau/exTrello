...
**reducer/lists.js**
Перетаскивание карточки внутри одного списка
```bash
  case 'MOVE_CARD':
        ...
        return res;
```
Перетаскивание карточки между списками
```bash
...
## Итого
TODO:
Не работает перетаскивание карточки в пустой список.
Не работает отмена событий с перетаскиванием в другой список. То есть, если событие hover поймали на другом списке, оно уже не отменяется, и если карточку утащили назад, она все равно будет добавлена.
Не все изменения состояний обрабатываются через Immutable.
